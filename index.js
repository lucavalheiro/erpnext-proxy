...
const ERP_HOST = process.env.ERP_HOST || "erpcoe.frappe.cloud";
const ERP_TOKEN = process.env.ERP_TOKEN || "";
const WAREHOUSE = process.env.WAREHOUSE || "Finished Goods - TD";
const PRICE_LIST = process.env.PRICE_LIST || "Standard Selling";
const COMPANY = process.env.COMPANY || "TDSynnex (Demo)";
...
function erpRequest(method, path, body, cb) {
  ...
  const req = https.request(options, (r) => {
    ...
    r.on("end", () => {
      try {
        cb(null, JSON.parse(d), r.statusCode);
      } catch (e) {
        cb(new Error("Parse error: " + d.slice(0, 300)), null, r.statusCode);
      }
    });
  });
...
function erpGet(path, cb) {
  erpRequest("GET", path, null, cb);
}

function erpPost(path, body, cb) {
  erpRequest("POST", path, body, cb);
}
...
.cart-btn{background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.3);color:#fff;padding:8px 16px;border-radius:8px;cursor:pointer;font-size:14px;display:flex;align-items:center;gap:8px}
...
.product-card{background:#fff;border:1.5px solid #e0e0e0;border-radius:12px;padding:16px;transition:border-color .15s,background .15s}
...
<div class="main">
  ...
  <div class="products" id="products">
    <p style="color:#888;font-size:14px">Carregando produtos...</p>
  </div>
...
fetch("/api/items")
  ...
  .then(function(d) {
    if (d.error) {
      document.getElementById("products").innerHTML =
        "<p style='color:#da1e28;font-size:13px'>Erro: " + d.error + "</p>";
      return;
    }
    items = d;
    renderProducts();
  ...
  .catch(function(e) {
    document.getElementById("products").innerHTML =
      "<p style='color:#da1e28;font-size:13px'>Falha: " + e.message + "</p>";
  });
...
function renderProducts() {
  ...
  items.forEach(function(p) {
    ...
    h += "<div class='product-card" + s + "' id='card-" + p.sku + "'>" +
      ...
      "<div class='product-stock " + (p.stock > 0 ? "stock-ok" : "stock-neg") + "'>" +
...
function renderCart() {
  ...
  document.getElementById("cartBadge").textContent =
    keys.length + " iten" + (keys.length !== 1 ? "s" : "");
...
function updateBtn() {
  document.getElementById("btnOrder").disabled =
    !document.getElementById("customer").value || !Object.keys(cart).length;
}
...
function criarOrdem() {
  ...
  fetch("/order", {
    ...
    .then(function(d) {
      if (d.success) {
        document.getElementById("status").innerHTML =
          "<div class='success-box'>Sales Order criado: <strong>" + d.order_id + "</strong></div>";
        cart = {};
        renderProducts();
        renderCart();
      } else {
        document.getElementById("status").innerHTML =
          "<div class='error-box'>Erro: " + d.error + "</div>";
        document.getElementById("btnOrder").disabled = false;
      }
    })
    .catch(function(e) {
      document.getElementById("status").innerHTML =
        "<div class='error-box'>Falha: " + e.message + "</div>";
      document.getElementById("btnOrder").disabled = false;
    });
}
</script>
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === "/api/items" && req.method === "GET") {
    if (!ERP_TOKEN) {
      ...
      res.end(JSON.stringify({ error: "ERP_TOKEN não configurado no Render." }));
      return;
    }
    ...
    erpGet(priceUrl, (err, priceData) => {
      if (err) {
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: "Erro preços: " + err.message }));
        return;
      }
      ...
      if (!prices.length) {
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: "Nenhum item encontrado na Price List." }));
        return;
      }
      ...
      erpGet(stockUrl, (err2, stockData) => {
        ...
        if (!err2 && stockData && stockData.data) {
          stockData.data.forEach((s) => {
            stockMap[s.item_code] = s.actual_qty;
          });
        }
  ...
  if (url.pathname === "/order" && req.method === "POST") {
    if (!ERP_TOKEN) {
      res.writeHead(500, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ success: false, error: "ERP_TOKEN não configurado no Render." }));
      return;
    }
    ...
    parseBody(req, (body) => {
      if (!body.customer || !body.items || !body.items.length) {
        ...
        res.end(JSON.stringify({ success: false, error: "customer e items obrigatórios." }));
        return;
      }
      ...
      erpPost("/api/resource/Sales Order", order, (err, data) => {
        ...
        console.log("[/order] resp:", JSON.stringify(data).slice(0, 500));
