...
<div class="main">
  ...
  <button class="btn-primary" id="btnOrder" onclick="criarOrdem()" disabled>
...
fetch('/api/items')
  ...
  .catch(function(e){
    document.getElementById('products').innerHTML =
      '<p style="color:#da1e28;font-size:13px">Falha de rede: ' + e.message + '</p>';
  });
...
function renderProducts(){
  if (!items.length) {
    document.getElementById('products').innerHTML =
      '<p style="color:#888;font-size:14px">Nenhum produto encontrado.</p>';
    return;
  }
  var html = '';
  items.forEach(function(p){
    ...
    html += '<div class="product-stock ' + (p.stock > 0 ? 'stock-ok' : 'stock-neg') + '">';
    html += p.stock > 0 ? 'Estoque: ' + p.stock : 'Sem estoque';
    html += '</div>';
...
function renderCart(){
  ...
  keys.forEach(function(k){
    ...
    html += '<div>';
    html += '<div style="font-size:14px;font-weight:500;color:#161616">' + c.name + '</div>';
    html += '<div style="font-size:12px;color:#888">' + c.qty + ' × ' + fmt(c.price) + '</div>';
    html += '</div>';
    html += '<div style="display:flex;align-items:center;gap:10px">';
    html += '<span style="font-weight:500;font-size:14px;color:#161616">' + fmt(sub) + '</span>';
    html += '<button class="remove-btn" onclick="removeItem(\'' + k + '\')">×</button>';
    html += '</div></div>';
  });
...
function updateBtn(){
  var ok = document.getElementById('customer').value && Object.keys(cart).length > 0;
...
function criarOrdem(){
  ...
  document.getElementById('status').innerHTML = '<p class="loading">⏳ Criando Sales Order no ERPNext...</p>';
  document.getElementById('btnOrder').disabled = true;

  var payload = {
    customer: customer,
    items: Object.keys(cart).map(function(k){
      return { sku: cart[k].sku, qty: cart[k].qty, price: cart[k].price };
    })
  };
  ...
  .then(function(d){
    if (d.success) {
      document.getElementById('status').innerHTML =
        '<div class="success-box">✅ Sales Order criado!<br><strong>' + d.order_id + '</strong></div>';
  ...
  .catch(function(e){
    document.getElementById('status').innerHTML =
      '<div class="error-box">❌ Falha de rede: ' + e.message + '</div>';
    document.getElementById('btnOrder').disabled = false;
  });
}
</script>
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/api/items' && req.method === 'GET') {
    const icons = {
      ...
      SKU005:'👟', SKU006:'☕', SKU007:'📺', SKU008:'🎒',
      SKU009:'🎧', SKU010:'📷'
    };
    const names = {
      SKU001:'T-shirt',    SKU002:'Laptop',      SKU003:'Book',
      SKU004:'Smartphone', SKU005:'Sneakers',    SKU006:'Coffee Mug',
      SKU007:'Television', SKU008:'Backpack',    SKU009:'Headphones',
      SKU010:'Camera'
    };
    ...
    erpGet(priceUrl, (err, priceData) => {
      if (err) {
        ...
        res.end(JSON.stringify({ error: 'Erro ao buscar preços: ' + err.message }));
        return;
      }
      ...
      if (!prices.length) {
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: 'Nenhum item na Price List "' + PRICE_LIST + '".' }));
        return;
      }
      ...
      erpGet(stockUrl, (err2, stockData) => {
        ...
        if (!err2 && stockData && stockData.data) {
          stockData.data.forEach(s => { stockMap[s.item_code] = s.actual_qty; });
        }
  ...
  if (url.pathname === '/order' && req.method === 'POST') {
    parseBody(req, (body) => {
      if (!body.customer || !body.items || !body.items.length) {
        ...
        res.end(JSON.stringify({ success: false, error: 'customer e items são obrigatórios.' }));
        return;
      }

      // delivery_date = hoje + 7 dias (ERPNext rejeita data igual a hoje)
      ...
      console.log('[/order] enviando para ERPNext:', JSON.stringify(order));

      erpPost('/api/resource/Sales%20Order', order, (err, data) => {
        if (err) {
          console.error('[/order] erro:', err.message);
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ success: false, error: err.message }));
          return;
        }

        console.log('[/order] resposta:', JSON.stringify(data).slice(0, 300));
